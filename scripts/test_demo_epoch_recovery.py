...
def main():
    ...
    with tempfile.TemporaryDirectory() as temp:
        ...
        authorization = create_filled_recovery_authorization(
            ...
            target_root=root / 'demo_execution_filled_recovery2',
        ...
        for label, adapter_value in (('residual', FilledRecoveryAdapter(amount='-63')), ('open_orders', FilledRecoveryAdapter(open_orders=[{'clientOrderId': 'unknown'}]))):
            ...
            blocked(lambda unsafe_auth=unsafe_auth, adapter_value=adapter_value: create_filled_recovery_authorization(predecessor_root=filled_predecessor, predecessor_checkpoint=filled_checkpoint_path, expected_predecessor_git_commit=PREDECESSOR_COMMIT, expected_predecessor_config_identity=cfg['config_identity'], target_git_commit=TARGET_COMMIT, target_config_identity=cfg['config_identity'], forward_root=root / 'forward_read_only', target_root=root / f'{label}_target', authorization_path=unsafe_auth, adapter=adapter_value, recovery_reason='unsafe', now=verified_at))
        ...
        create_filled_recovery_authorization(predecessor_root=stale_predecessor, predecessor_checkpoint=stale_path, expected_predecessor_git_commit=PREDECESSOR_COMMIT, expected_predecessor_config_identity=cfg['config_identity'], target_git_commit=TARGET_COMMIT, target_config_identity=cfg['config_identity'], forward_root=root / 'forward_read_only', target_root=root / 'stale_target', authorization_path=stale_auth, adapter=FilledRecoveryAdapter(), recovery_reason='approved', now=verified_at)
        ...
        create_filled_recovery_authorization(predecessor_root=bound_predecessor, predecessor_checkpoint=bound_path, expected_predecessor_git_commit=PREDECESSOR_COMMIT, expected_predecessor_config_identity=cfg['config_identity'], target_git_commit=TARGET_COMMIT, target_config_identity=cfg['config_identity'], forward_root=root / 'forward_read_only', target_root=root / 'changed_checkpoint_target', authorization_path=bound_auth, adapter=FilledRecoveryAdapter(), recovery_reason='approved', now=verified_at)
        ...
        blocked(lambda: create_filled_recovery_authorization(predecessor_root=ambiguous_predecessor, predecessor_checkpoint=ambiguous_path, expected_predecessor_git_commit=PREDECESSOR_COMMIT, expected_predecessor_config_identity=cfg['config_identity'], target_git_commit=TARGET_COMMIT, target_config_identity=cfg['config_identity'], forward_root=root / 'forward_read_only', target_root=root / 'ambiguous_target', authorization_path=root / 'ambiguous.authorization.json', adapter=FilledRecoveryAdapter(), recovery_reason='unsafe', now=verified_at))
        ...
        create_filled_recovery_authorization(predecessor_root=write_failure_predecessor, predecessor_checkpoint=write_failure_path, expected_predecessor_git_commit=PREDECESSOR_COMMIT, expected_predecessor_config_identity=cfg['config_identity'], target_git_commit=TARGET_COMMIT, target_config_identity=cfg['config_identity'], forward_root=root / 'forward_read_only', target_root=root / 'filled_write_failure_target', authorization_path=write_failure_auth, adapter=FilledRecoveryAdapter(), recovery_reason='approved', now=verified_at)
        ...
        # A durable claim survives a pre-publication failure and permits only
        # a retry of its originally authorized target, never a target swap.
        retried = create_filled_recovery_epoch(predecessor_root=write_failure_predecessor, predecessor_checkpoint=write_failure_path, expected_predecessor_git_commit=PREDECESSOR_COMMIT, expected_predecessor_config_identity=cfg['config_identity'], target_root=root / 'filled_write_failure_target', target_git_commit=TARGET_COMMIT, target_config_identity=cfg['config_identity'], forward_root=root / 'forward_read_only', authorization_path=write_failure_auth, now='2026-09-15T00:04:00+00:00')
        published = json.loads(next((root / 'filled_write_failure_target' / 'recovery').glob('*/*.jsonl')).read_text(encoding='utf-8'))
        claim = json.loads((root / 'write_failure.authorization.json.claim').read_text(encoding='utf-8'))
        assert published['filled_recovery_authorization_identity'] == retried['authorization_identity'] == claim['authorization_identity'] and published['filled_recovery_authorization_claim_identity'] == claim['claim_identity']
        blocked(lambda: create_filled_recovery_epoch(predecessor_root=write_failure_predecessor, predecessor_checkpoint=write_failure_path, expected_predecessor_git_commit=PREDECESSOR_COMMIT, expected_predecessor_config_identity=cfg['config_identity'], target_root=root / 'swapped_target', target_git_commit=TARGET_COMMIT, target_config_identity=cfg['config_identity'], forward_root=root / 'forward_read_only', authorization_path=write_failure_auth, now='2026-09-15T00:04:00+00:00'))
    ...
    print('DEMO_FILLED_RECOVERY_TARGET_BOUND_SINGLE_USE_CLAIM=PASS')
    print('OOS_READS=0')
