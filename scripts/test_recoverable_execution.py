...
def main():
 ...
 with tempfile.TemporaryDirectory() as td:
  ...
  rejected(lambda:claim_task(root,manifest,n7.plan,second['task_identity']))
  state_path=root/'run_state.json';crashed=json.loads(state_path.read_text());crashed['tasks'][second['task_identity']]['lease_expires_at']=0;state_path.write_text(json.dumps(crashed));claim_task(root,manifest,n7.plan,second['task_identity'],owner='recovery');state=load_run(root,manifest,n7.plan);assert any(item['status']=='STALE_RECOVERED' for item in state['tasks'][second['task_identity']]['attempts'])
  ...
  rejected(lambda:complete_task(root,manifest,n7.plan,bad_complete(expected_ids[:-1],expected_ids[:k],['v'+str(i) for i in range(k)])))
  rejected(lambda:complete_task(root,manifest,n7.plan,bad_complete(expected_ids[:-1]+['unknown'],expected_ids[:k],['v'+str(i) for i in range(k)])))
  rejected(lambda:complete_task(root,manifest,n7.plan,bad_complete([expected_ids[0]]*len(expected_ids),expected_ids[:k],['v'+str(i) for i in range(k)])))
  rejected(lambda:complete_task(root,manifest,n7.plan,bad_complete(expected_ids,['not-train']*k,['v'+str(i) for i in range(k)])))
  rejected(lambda:complete_task(root,manifest,n7.plan,bad_complete(expected_ids,expected_ids[:k],['v']*k)))
 ...
 with tempfile.TemporaryDirectory() as td:
  ...
  for task in reversed(n7.plan['tasks']):
   claim_task(root,local_manifest,n7.plan,task['task_identity']);state=load_run(root,local_manifest,n7.plan);model=next(row for row in n7.plan['models'] if row['model_id']==task['model_id']);top=[f'{task["task_identity"]}:{index}' for index in range(min(n7.plan['top_k_train'],model['grid_combinations']))]
   train_ids=_expected_train_ids(state,task);complete_task(root,local_manifest,n7.plan,task_result_payload(state,n7.plan,task['task_identity'],status='COMPLETED',train_evaluations=model['grid_combinations'],validation_evaluations=len(top),train_result_identities=train_ids,selected_train_top_k=train_ids[:len(top)],validation_result_identities=[f'{task["task_identity"]}-validation-{i}' for i in range(len(top))]))
  one=final_result_identity(root,local_manifest,n7.plan);assert one and not unfinished_task_ids(root,local_manifest,n7.plan)
 with tempfile.TemporaryDirectory() as td:
  repo=Path(td)/'repo';repo.mkdir();local_manifest=build_manifest(n7,n8,source_git_commit=clean_repo(repo),worker_config={'workers':2},output_destination='data/reports/formal_runs/N10_RESULT.json',created_at='synthetic')
  root=Path(td)/'failed';initialize_run(root,local_manifest,n7.plan,n7=n7,n8=n8,repo_root=repo);task=n7.plan['tasks'][0];claim_task(root,local_manifest,n7.plan,task['task_identity']);fail_task(root,local_manifest,n7.plan,task['task_identity'],'SyntheticError','synthetic failure');failed=load_run(root,local_manifest,n7.plan);assert any(item['status']=='FAILED' and item['error_message']=='synthetic failure' for item in failed['tasks'][task['task_identity']]['attempts']);claim_task(root,local_manifest,n7.plan,task['task_identity'],owner='retry');retried=load_run(root,local_manifest,n7.plan);history=retried['tasks'][task['task_identity']]['attempts'];assert any(item['status']=='FAILED' for item in history) and any(item['status']=='RETRY_AUTHORIZED' for item in history);assert task['task_identity'] in unfinished_task_ids(root,local_manifest,n7.plan);rejected(lambda:final_result_identity(root,local_manifest,n7.plan))
