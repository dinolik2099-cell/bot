...
class Session:
 ...
 def get(self,url,timeout):
  ...
  if '/klines?' in url:
   ...
   return Response([[int((datetime(2026,9,1,tzinfo=timezone.utc)+timedelta(hours=i)).timestamp()*1000),'1','1','1','1','1',int((datetime(2026,9,1,tzinfo=timezone.utc)+timedelta(hours=i+1)-timedelta(milliseconds=1)).timestamp()*1000)] for i in range(2)])
...
def main():
 with tempfile.TemporaryDirectory() as root:
  ...
  model={'model_id':'m','parameter_grid_hash':'g'*64,'strategy_function_hash':'s'*64,'implementation_module_hash':'i'*64,'warmup_bars':1}
