...
def main():
 p=argparse.ArgumentParser();p.add_argument('--config',required=True);p.add_argument('--status',action='store_true');p.add_argument('--diagnostics',action='store_true');p.add_argument('--checkpoint');p.add_argument('--preflight',action='store_true');p.add_argument('--serve',action='store_true');p.add_argument('--plan');p.add_argument('--declarations');p.add_argument('--data-root',default='data/forward_research');p.add_argument('--repo-root',default='.');p.add_argument('--upgrade-resume',action='store_true');p.add_argument('--approved-predecessor-commit');args=p.parse_args();config=validate_config(args.config);runtime=ForwardRuntime()
 if args.upgrade_resume != bool(args.approved_predecessor_commit):p.error('--upgrade-resume and --approved-predecessor-commit must be supplied together')
 if args.upgrade_resume and not args.serve:p.error('--upgrade-resume is only valid with --serve')
 if args.preflight:
 ...
 if args.serve:
  ...
  authority=build_production_authority(config_path=args.config,plan_path=args.plan,declaration_path=args.declarations,data_root=args.data_root,checkpoint_path=args.checkpoint or 'data/forward_research/checkpoints/runtime.json',repo_root=args.repo_root,upgrade_resume=args.upgrade_resume,approved_predecessor_commit=args.approved_predecessor_commit);authority.serve();return 0
