#!/usr/bin/env python3
"""QuantBot Phase 2.4-B V1.3.3 夜间多模型/多方向最坏情况耐久压力测试。
...
def main():
    ...
    out=ROOT/'data/reports/phase2_4_b_overnight_torture_v133.jsonl'; out.parent.mkdir(parents=True,exist_ok=True)
    summary=ROOT/'data/reports/phase2_4_b_overnight_torture_summary_v133.md'
    ...
    lines=['# Phase 2.4-B V1.3.3 夜间多模型/多方向最坏情况压力测试','','状态：完成','',f'- 总运行时间：{(time.time()-start)/3600:.2f} 小时',f'- 轮次：{rnd}',f'- 共享资金回测次数：{total}',f'- 审计失败记录：{audit_fail}','- OOS/D-2/D-3：未读取','- 选模：未使用 OOS；不按最好收益筛选','', '## 各组合最坏共享资金结果']
    ...
    summary.write_text('\n'.join(lines)+'\n',encoding='utf-8')
    print('='*72); print('PHASE2_4_B_OVERNIGHT_TORTURE_V1.3.3_OK'); print(f'轮次={rnd} 共享回测={total} 审计失败={audit_fail}'); print(f'JSONL={out}'); print(f'SUMMARY={summary}')
