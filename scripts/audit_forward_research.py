...
def audit_forward_evidence(root,days):
 root=Path(root);files=sorted(root.rglob('*.jsonl'));counts=Counter();chains=Counter();snapshots=set();records=0
 for path in files:
  ...
  for raw in path.open(encoding='utf-8'):
   ...
   if row.get('schema_version')=='quantbot-forward-record-v1':
    ...
    chains[(row['git_commit'],row['config_identity'])]+=1;snapshots.add(row['universe_identity'])
 if len(chains)>1:raise ForwardResearchError('forward_evidence_provenance_chain_drift')
 out={'schema_version':'quantbot-forward-research-audit-v3','days':days,'files':len(files),'records':records,**dict(sorted(counts.items())),'provenance_chains':len(chains),'universe_snapshots':len(snapshots),'forward_evidence_only':True,'model_ranking_updated':False,'frozen_selection_modified':False,'oos_read':False}
