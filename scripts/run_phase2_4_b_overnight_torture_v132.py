#!/usr/bin/env python3
"""QuantBot Phase 2.4-B V1.3.2 夜间多模型/多方向最坏情况耐久压力测试。
...
BASE=ROOT/'scripts'/'run_phase2_4_b_shared_capital_backtest_v132.py'
spec_name='p24b_v132_base'
...
def worker(payload):
    ...
    for w in ('TRAIN','VALIDATION'):
        ...
        for m in needed:
            ...
            sigs[(m,symbol)]=P.build_signal_map(frame,spec.strategy,rec['params'],f'P24B132:{m}:{symbol}:{w}')
...
def main():
    ...
    out=ROOT/'data/reports/phase2_4_b_overnight_torture_v132.jsonl'; out.parent.mkdir(parents=True,exist_ok=True)
    summary=ROOT/'data/reports/phase2_4_b_overnight_torture_summary_v132.md'
                ...
                for window in ('TRAIN','VALIDATION'):
                    for sn,fm,sm in SCENARIOS:
                        ...
                        rec={'轮次':rnd,'组合':recipe,'模型数量':len(set(m for m,_ in keys)),'Sleeve数量':len(keys),'窗口':window,'场景':sn,
                             '收益':r['total_return'],'最大回撤':r['max_drawdown'],'盈利因子':r['profit_factor'],
                             '交易次数':len(r['trades']),'拒绝入场':r['rejected_entries'],
                             '风险限制拒绝':r.get('risk_limit_rejections',0),
                             '最大连续亏损':r.get('max_consecutive_losses',0),
                             '最低资金':r.get('min_equity',r['final_equity']),
                             '原始最低资金':r.get('min_raw_equity',r['final_equity']),
                             '破产':bool(r.get('bankruptcy',False)),
                             '最终资金':r['final_equity'],'审计通过':not errs}
    ...
    lines=['# Phase 2.4-B V1.3.2 夜间多模型/多方向最坏情况压力测试','','状态：完成','',f'- 总运行时间：{(time.time()-start)/3600:.2f} 小时',f'- 轮次：{rnd}',f'- 共享资金回测次数：{total}',f'- 审计失败记录：{audit_fail}','- OOS/D-2/D-3：未读取','- 选模：未使用 OOS；不按最好收益筛选','', '## 各组合最坏共享资金结果']
    for recipe in [x[0] for x in RECIPES]:
        sub=rows[rows['组合']==recipe]
        if sub.empty: continue
        worst=sub.loc[sub['最大回撤'].idxmax()]; low=sub.loc[sub['最终资金'].idxmin()]
        lines += [f'### {recipe}',f'- 模型数：{int(sub.iloc[0]["模型数量"])}，Sleeve数：{int(sub.iloc[0]["Sleeve数量"])}',f'- 历史测试最大回撤上界样本：{worst["最大回撤"]:.2%}（{worst["窗口"]}/{worst["场景"]}）',f'- 最低最终资金样本：{low["最终资金"]:.2f}（{low["窗口"]}/{low["场景"]}）',
                  f'- 最低资金样本：{sub["最低资金"].min():.2f}',
                  f'- 原始最低资金样本：{sub["原始最低资金"].min():.2f}',
                  f'- 破产样本数：{int(sub["破产"].sum())}',
                  f'- 最差PF样本：{sub["盈利因子"].min():.3f}']
    lines += ['', '## 判定原则','', '本轮不是选最高收益；优先判断本金破坏、最大回撤、连续亏损、风险上限执行和多方向同步冲击。','只有完成后续组合冻结、独立 OOS 后，才讨论正式交易授权。']
    summary.write_text('\n'.join(lines)+'\n',encoding='utf-8')
    print('='*72); print('PHASE2_4_B_OVERNIGHT_TORTURE_V1.3.1_OK'); print(f'轮次={rnd} 共享回测={total} 审计失败={audit_fail}'); print(f'JSONL={out}'); print(f'SUMMARY={summary}')
