#!/usr/bin/env python3
"""Phase 2.4-B V1.3.4 focused regression tests."""
...
TARGET = ROOT / "scripts" / "run_phase2_4_b_overnight_torture_v134.py"
spec = importlib.util.spec_from_file_location("p24b_v134", TARGET)
mod = importlib.util.module_from_spec(spec)
spec.loader.exec_module(mod)


def main():
    quick = mod.get_run_plan('quick')
    full = mod.get_run_plan('full')
    assert len(quick[0]) == 1 and len(quick[1]) == 2 and quick[2] == ('TRAIN','VALIDATION') and quick[3] is False
    assert len(full[0]) == 4 and len(full[1]) == 5 and full[3] is True
    ...
    print("PHASE2_4_B_OVERNIGHT_TORTURE_V1.3.4_TEST_OK")
