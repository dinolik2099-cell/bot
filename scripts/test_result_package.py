...
def main():
    ...
    altered=copy.deepcopy(pkg);altered["counts"]["train"]=999;altered["artifact_identity"]=seal({key:value for key,value in altered.items() if key!="artifact_identity"})["artifact_identity"];reject(lambda:validate_evidence_package(altered,expected_run_id="r"*64,expected_manifest_identity="m"*64))
