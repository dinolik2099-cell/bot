...
def shared_backtest(frames, signal_maps, recipe_keys, boundary, initial=INITIAL_EQUITY):
    ...
    for ts in timeline:
        ...
        for symbol, pos in list(positions.items()):
            ...
            if stop_hit or target_hit:
                ...
                raw_equity = equity + tr.net_pnl
                min_raw_equity = min(min_raw_equity, raw_equity)
                equity = max(0.0, raw_equity)
                trades.append(tr)
    ...
    dd=np.max(np.where(peak > 0, 1.0 - vals/peak, 0.0)) if len(vals) else 0.0
    ...
    return {
        ...
        "halted": bool(halted), "halt_reason": halt_reason,
        "bankruptcy": bool(halted and halt_reason == "破产"),
        "bankruptcy_events": int(bankruptcy_events),
        "min_equity": float(np.min(vals)) if len(vals) else float(equity),
        "min_raw_equity": float(min_raw_equity),
        "risk_limit_rejections": int(risk_limit_rejections),
