...
def test_boundary_aware_loader_does_not_validate_gaps():
    frame = make_frame()
    ...
    with patch(
        ...
        result = load_boundary_aware_symbol(
            "/raw",
            "SOLUSDT",
            "1h",
            market="um",
...
def test_parquet_fallback(tmp: Path):
    ...
    with patch(
        ...
        loaded, source = load_boundary_aware_symbol_with_parquet_fallback(
            tmp / "raw",
            tmp / "parquet",
            "SOLUSDT",
            "1h",
...
def main():
    ...
    test_boundary_aware_loader_does_not_validate_gaps()
    print("PHASE2_1_V1_1_1_CANONICAL_LOADER_TEST_OK")
