...
class DemoRuntime:
 ...
 def _health(self,signal):
  ...
  active_positions=[row for row in positions if float(row.get('positionAmt',0)) != 0]
  # V1 has no remote position-to-model attribution.  Treat an attributed
  # strategy exposure as unknown rather than letting a hard-coded zero bypass
  # the policy.
  if active_positions:raise FailClosedError('demo_strategy_exposure_unattributable')
  unresolved=self.ledger.unresolved();gross_remote=sum(abs(float(row.get('positionAmt',0))*float(row.get('markPrice',0))) for row in positions)
  gross_ledger=sum(float(row['intent']['notional']) for row in unresolved)
  strategy_ledger=sum(float(row['intent']['notional']) for row in unresolved if row['intent'].get('model_id')==signal.get('model_id'))
 ...
 def consume_once(self,engine,dry_run=True):
  ...
  for marker,signal in ForwardSignalReader(self.forward_root).discover(cursor,self.epoch_start):
   ...
   try:
    price=float(self.adapter.ticker_price(signal['symbol'])['price']);result=engine.process(signal,signal['created_at'][:10],dry_run=dry_run,price=price,filters=_filters(exchange,signal['symbol']),health=self._health(signal))
