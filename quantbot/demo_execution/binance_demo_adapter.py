...
import hashlib,hmac,json,socket,time
...
class BinanceDemoAdapter:
 ...
 def _request_error(self,exc):
  """Classify only known temporary transport failures as retryable.

  Error text intentionally contains no URL, query, credentials, or signature.
  """
  if isinstance(exc,HTTPError):
   status=int(exc.code);code=None
   try:
    payload=json.loads(exc.read(4096).decode('utf-8','replace'));code=payload.get('code') if isinstance(payload,dict) else None
 ...
 def _request(self,method,path,params=None,signed=False):
  ...
  if self.transport:
   ...
   except DemoExecutionError:raise
   except Exception as exc:raise self._request_error(exc) from exc
