...
def universe_snapshot(rows:Sequence[UniverseSymbol],timestamp:str):
 assert_shadow_only();eligible=sorted((asdict(row) for row in rows if row.eligible()),key=lambda row:row['symbol']);membership={'schema_version':'quantbot-forward-universe-membership-v1','symbols':[row['symbol'] for row in eligible],'threshold_quote_volume':3_000_000,'source':'binance_public'};payload={'schema_version':'quantbot-forward-universe-v1','timestamp':timestamp,'symbols':eligible,'threshold_quote_volume':3_000_000,'source':'binance_public','forward_research_only':True,'membership_identity':identity(membership)};payload['universe_identity']=identity(payload);return payload
