...
def make_strategy_adapter(
    ...
    *,
    ...
    if not isinstance(full_frame.index, pd.DatetimeIndex):
        raise TypeError("full frame index must be DatetimeIndex")
    if full_frame.index.tz is None:
        raise ValueError("full frame index must be timezone-aware")
    if not full_frame.index.is_monotonic_increasing:
        raise ValueError("full frame timestamps are not monotonic")
    if full_frame.index.has_duplicates:
        raise ValueError("full frame contains duplicate timestamps")

    # Precompute the complete strategy frame exactly once.
    evaluated = strategy(full_frame.copy(), **params)

    if not isinstance(evaluated, pd.DataFrame):
        raise TypeError("strategy must return a pandas DataFrame")
    if not isinstance(evaluated.index, pd.DatetimeIndex):
        raise TypeError("strategy result index must be DatetimeIndex")
    if evaluated.index.tz is None:
        raise ValueError("strategy result index must be timezone-aware")
    if not evaluated.index.is_monotonic_increasing:
        raise ValueError("strategy result timestamps are not monotonic")
    if evaluated.index.has_duplicates:
        raise ValueError("strategy result contains duplicate timestamps")

    required = {"signal", "stop", "target"}
    missing = required - set(evaluated.columns)
    if missing:
        raise ValueError(
            f"strategy result missing columns: {sorted(missing)}"
        )

    if len(evaluated) != len(full_frame):
        raise ValueError(
            "strategy result length must equal full frame length "
            "for precomputed adapter"
        )

    def adapter(history: pd.DataFrame, i: int) -> Signal | None:
        if i < 0 or i >= len(full_frame):
            raise IndexError(
                f"strategy execution index out of range: {i}"
            )

        if not isinstance(history.index, pd.DatetimeIndex):
            raise TypeError(
                "strategy history index must be DatetimeIndex"
            )
        if history.index.tz is None:
            raise ValueError(
                "strategy history index must be timezone-aware"
            )
        if not history.index.is_monotonic_increasing:
            raise ValueError(
                "strategy history timestamps are not monotonic"
            )
        if history.index.has_duplicates:
            raise ValueError(
                "strategy history contains duplicate timestamps"
            )

        if len(history) != i:
            raise ValueError(
                f"engine history length mismatch: expected {i}, "
                f"got {len(history)}"
            )

        execution_ts = full_frame.index[i]

        if history.empty:
            return None

        if history.index[-1] >= execution_ts:
            raise ValueError(
                f"lookahead violation: history reaches execution "
                f"timestamp {execution_ts}"
            )

        # Signal at T may only use the completed T-1 strategy row.
        ...
        if side not in (-1, 1):
            raise ValueError(
                f"invalid strategy signal: {side}"
            )
