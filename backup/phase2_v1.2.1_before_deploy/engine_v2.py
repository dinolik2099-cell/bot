...
class BacktestEngine:
    """
    PHASE 2 V1.2 conservative OHLC event engine.
    ...
    Gap accounting is intentionally explicit:
      - gap_bars_seen = every configured gap timestamp that exists on the
        engine timeline;
      - skipped_gap_bars = gap timestamps where the symbol would otherwise
        reach the entry-evaluation stage. No strategy call occurs there.
    """
    ...
    def run(
        ...
        gap_timeline = {
            symbol: gaps & set(timeline)
            for symbol, gaps in self.gap_indices.items()
        }
        ...
        for ts in timeline:
            ...
            for symbol, df in data.items():
                ...
                # A configured gap is a hard non-tradable point. This check is
                # deliberately before any strategy invocation.
