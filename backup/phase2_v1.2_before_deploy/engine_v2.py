...
class BacktestEngine:
    """
    PHASE 2 V1.1 conservative OHLC event engine.

    IMPORTANT EXECUTION CONTRACT
    ----------------------------
    1. At execution timestamp T, strategy receives history strictly before T.
    2. A signal returned for T executes at T OPEN.
    3. The T candle is NOT eligible for SL/TP after an entry at T OPEN.
       SL/TP checking begins on the next available candle for that symbol.
    4. If a later candle touches both SL and TP, OHLC data cannot reveal
       intrabar order. The engine resolves the ambiguity conservatively:
       STOP wins.
    5. Missing/gap timestamps supplied through gap_indices are non-tradable
       for entries. They are never synthesized.
    6. A position can remain open across a missing timestamp; the engine does
       not invent a price during the gap. Exit checks resume at the next
       actual candle.
    7. At the end of data, remaining positions are closed at the final
       available close.
    """
    ...
    def run(
        ...
        if not data or set(data) != set(strategies):
            raise ValueError(
                "data and strategies must contain the same non-empty symbols"
            )
        ...
        for ts in timeline:
            ...
            # 1) Existing positions are eligible for exit only if this is
            # strictly after their entry bar.
            for symbol, pos in list(positions.items()):
                ...
                if stop_hit or target_hit:
                    ...
                    trade = self._close_position(
                        pos, ts, float(reference), reason
                    )
                    equity += trade.net_pnl
                    trades.append(trade)
                    del positions[symbol]

            # 2) New entries are generated from strict pre-T history.
            for symbol, df in data.items():
                ...
                position_fraction = (
                    signal.position_fraction or self.max_position_fraction
                )
                risk_fraction = min(
                    float(risk_fraction), self.max_risk_fraction
                )
                position_fraction = min(
                    float(position_fraction), self.max_position_fraction
                )
                ...
                qty_by_notional = (
                    equity * position_fraction / entry_reference
                )
            ...
            # 3) Mark-to-market. No synthetic prices are used.
            mtm = equity
        ...
        # 4) Explicit end-of-data close.
        for symbol, pos in list(positions.items()):
            df = data[symbol]
            last_ts = _utc(df.index[-1])
            last_close = float(df.iloc[-1]["close"])
            trade = self._close_position(
                pos, last_ts, last_close, "end_of_data"
            )
            equity += trade.net_pnl
            trades.append(trade)
            del positions[symbol]
        ...
        curve_series = curve_series[
            ~curve_series.index.duplicated(keep="last")
        ].sort_index()
    ...
    def _close_position(
        ...
        exit_price = self.cost_model.execution_price(
            reference_price, exit_side
        )
