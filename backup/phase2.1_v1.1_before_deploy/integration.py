...
def load_canonical_symbol(
    ...
    *,
    ...
    """
    Canonical QuantBot data entry point.

    Deliberately delegates CSV normalization, shard merging, duplicate
    removal, ordering and validation to quantbot.data.load.load_symbol().
    """
    return load_symbol(
        raw_root,
        symbol,
...
def load_symbol_with_parquet_fallback(
    ...
    *,
    ...
    """
    Prefer the canonical raw loader. If the canonical raw dataset is absent,
    use an existing single-file Parquet dataset.

    No conversion or synthetic candle generation is performed here.
    """
    try:
        return (
