...
def main():
    ...
    locked_symbols = set(args.symbols)
    gap_symbols = {gap.symbol for gap in dataset.gaps}
    if not gap_symbols.issubset(locked_symbols):
        raise SystemExit(
            "GAP_SYMBOL_MISMATCH: gap symbols are not in CLI universe"
        )
    ...
    print("=" * 72)
    print("QuantBot PHASE 2.1 Boundary × Backtest Integration V1.0.1")
    print("=" * 72)
    ...
    print(f"effective_end: {expected_end(dataset).isoformat()}")
    ...
    print("PHASE2_1_V1_0_1_INTEGRATION_OK")
